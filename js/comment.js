...
var MOOMOO = window.MOOMOO || {};
;(function (JL) {
  var constants = {
    ACCESS_TOKEN_KEY: 'github-comments-token', // access_token key
    USER_INFO_KEY: 'github-user-info',         // 登录用户信息 key
    PER_PAGE: 5,                                    // 每页的评论数
  ...
  JL.Renders = {
    box: {
      tpl: [
        '<section class="box" id="MOOMOO__commentBox">',
          '<div class="com-avatar"><img id="MOOMOO__loginAvatar" src="/img/unsigned_avatar.jpg" alt="avatar"></div>',
          '<div class="com-text">',
            '<div class="main">',
              '<textarea class="text-area-edited show" id="MOOMOO__editBox" placeholder="欢迎评论！"></textarea>',
              '<div class="text-area-preview" id="MOOMOO__previewBox"></div>',
            '</div>',
            '<div class="switch">',
              '<div class="switch-item on" id="MOOMOO__editSwitcher" onclick="MOOMOO.Actions.editPreviewSwitch(\'edit\')">编辑</div>',
              '<div class="switch-item" id="MOOMOO__previewSwitcher" onclick="MOOMOO.Actions.editPreviewSwitch(\'preview\')">预览</div>',
            '</div>',
            '<div class="button" onclick="MOOMOO.Actions.postComment()">提交</div>',
          '</div>',
        '</section>'
      ].join(''),
      update: function () {
        ...
        // 默认头像路径 /img/MOOMOO.jpg
        $('MOOMOO__loginAvatar').src = userInfo.avatar_url || '/img/unsigned_avatar.jpg';
      }
    },
    list: {
      tpl: [
        '<section class="list-wrap" id="MOOMOO__commentList">',
      ...
      update: function (page, comments, list, callback) {
        ...
        if (comments === 0) {
            ...
            item = [
              '<li class="item">',
                ...
                '<div class="user-comment">',
                  '<div class="user-comment-header" id="MOOMOO__comment_' + list[i].id + '_reactions">',
                    ...
                    '<span class="like" onclick="MOOMOO.Actions.like(' + list[i].id + ')">点赞</span>',
                    '<span class="like-num">' + list[i].reactions.heart + '</span>',
                    '<span class="reply" onclick="MOOMOO.Actions.reply(\'' + list[i].user.login + '\', \'' + (list[i].body_html || list[i].body).replace(/<[^>]+>|\s|[\r\n]/g, ' ') + '\')">回复</span>',
                  '</div>',
          ...
          if (allPages === 1) {
            ...
            for (var i = 1; i <= allPages; i++) {
              if (i === page) {
                ...
                pageItem = '<a href="javascript: MOOMOO.Actions.pageJump(' + i + ');" class="item">' + i + '</a>';
              }
              pageList.push(pageItem);
            }
            if (page !== 1) {
              pageList.unshift('<a href="javascript: MOOMOO.Actions.pageJump(' + (page - 1) + ');" class="item">上页</a>');
            }
            if (page !== allPages) {
              pageList.push('<a href="javascript: MOOMOO.Actions.pageJump(' + (page + 1) + ');" class="item">下页</a>');
            }
          } else if (allPages > perNavPageMaxSize) {
            if (page <= perNavPageMaxSize) {
              ...
              pageList.push('<a href="javascript: MOOMOO.Actions.pageJump(' + (page + 1) + ');" class="item">下页</a>');
              pageList.push('<a href="javascript: MOOMOO.Actions.pageJump(' + allPages + ');" class="item">末页</a>');
              ...
              pageList.push('<a href="javascript: MOOMOO.Actions.pageJump(1);" class="item">首页</a>');
              pageList.push('<a href="javascript: MOOMOO.Actions.pageJump(' + (page - 1) + ');" class="item">上页</a>');
          ...
          html = [
            '<header class="list-header">总共 <span class="comments-num" id="MOOMOO__commentsNum">' + JL.issueComments + '</span> 条评论</header>',
            '<ul class="list">',
              htmlList.join(''),
            '</ul>',
            '<div class="page-nav">',
        ...
        $('MOOMOO__commentList').innerHTML = html;
      ...
      reactionUpdate: function (commentId, reactions) {
        ...
        for (var i = 0, len = reactions.length; i < len; i++) {
          if (userId === reactions[i].user.id) {
            ...
            addClass($('MOOMOO__comment_' + commentId + '_reactions').getElementsByClassName('like')[0], 'liked');
            $('MOOMOO__comment_' + commentId + '_reactions').getElementsByClassName('like')[0].innerHTML = '已赞';
            break;
          }
        }
      },
      addOne: function (data) {
        ...
        var item = [
          ...
          '</div>',
          '<div class="user-comment">',
            '<div class="user-comment-header" id="MOOMOO__comment_' + data.id + '_reactions">',
              '<span class="post-name">' + data.user.login +  '</span>',
              '<span class="post-time">' + formatDate('yyyy-MM-dd hh:mm', new Date(data.created_at)) + '</span>',
              '<span class="like" onclick="MOOMOO.Actions.like(' + data.reactions.heart + ')">点赞</span>',
              '<span class="like-num">' + data.reactions.heart + '</span>',
              '<span class="reply" onclick="MOOMOO.Actions.reply(\'' + data.user.login + '\', \'' + (data.body_html || data.body).replace(/<[^>]+>|\s|[\r\n]/g, ' ') + '\')">回复</span>',
            '</div>',
            '<div class="user-comment-body">' + (data.body_html || data.body) + '</div>',
          '</div>'
        ].join('');
        oLi.innerHTML = item;
        var oUl = $('MOOMOO__commentList').getElementsByTagName('ul')[0];
        if (oUl) {
          oUl.insertBefore(oLi, oUl.firstChild);
          $('MOOMOO__commentsNum').innerHTML = JL.issueComments + 1;
        } else {
          $('MOOMOO__commentList').innerHTML = [
            '<header class="list-header">总共 <span class="comments-num" id="MOOMOO__commentsNum">' + (JL.issueComments + 1) + '</span> 条评论</header>',
            '<ul class="list">',
              '<li class="item">',
                item,
              '</li>',
    ...
    signBar: {
      tpl: [
        '<div class="sign-bar" id="MOOMOO__commentSignBar">',
        '</div>'
      ].join(''),
      update: function () {
        ...
        if (token && userInfo) {
          ...
          html = [
            ...
            '<span class="sign-link" onclick="MOOMOO.Actions.signOut()">退出</span>'
          ].join('');
        } else {
          html = [
        ...
        $('MOOMOO__commentSignBar').innerHTML = html;
      }
    },
    tips: {
      tpl: '<section class="tips clearfix" id="MOOMOO__comment_tips">注：评论支持 markdown 语法！</section>',
      update: function () {
        var userInfo = localStorage.getItem(constants.USER_INFO_KEY);
        var handler = '';
        // 如果文章还没关联 issue 并且登录账号是自己时
        if (userInfo && JSON.parse(userInfo).login === JL.options.owner && JL.issueNumber === 0) {
          handler = '<a href="javascript: MOOMOO.Actions.createIssue();" class="init" title="文章关联 issue">初始化评论</a>';
        }
        $('MOOMOO__comment_tips').innerHTML = handler + '注：评论支持 markdown 语法！';
      }
    },
    ...
    loading: {
      create: function (oParent) {
        ...
        oLoading.id = 'MOOMOO__loadingMask';
      ...
      remove: function () {
        var oLoading = $('MOOMOO__loadingMask');
  ...
  JL.Actions = {
    ...
    editPreviewSwitch: function (flag) {
      if (flag === 'edit') {
        removeClass('MOOMOO__previewSwitcher', 'on');
        addClass('MOOMOO__editSwitcher', 'on');
        removeClass('MOOMOO__previewBox', 'show');
        addClass('MOOMOO__editBox', 'show');
      } else {
        removeClass('MOOMOO__editSwitcher', 'on');
        addClass('MOOMOO__previewSwitcher', 'on');
        removeClass('MOOMOO__editBox', 'show');
        addClass('MOOMOO__previewBox', 'show');
        var text = $('MOOMOO__editBox').value.trim();
        // 安全转义
        ...
        if (text) {
          JL.Requests.markdown({
            ...
            $('MOOMOO__previewBox').innerHTML = res;
          });
        } else {
          $('MOOMOO__previewBox').innerHTML = '';
        }
      }
    },
    postComment: function () {
      ...
      var body = $('MOOMOO__editBox').value.trim();
      // 安全转义
    ...
    like: function (commentId) {
      var oLiked = $('MOOMOO__comment_' + commentId + '_reactions').getElementsByClassName('liked');
      var oLike = $('MOOMOO__comment_' + commentId + '_reactions').getElementsByClassName('like')[0];
      var oNum = $('MOOMOO__comment_' + commentId + '_reactions').getElementsByClassName('like-num')[0];
      var accessToken = localStorage.getItem(constants.ACCESS_TOKEN_KEY);
      var userInfo = localStorage.getItem(constants.USER_INFO_KEY);
      if (oLiked.length) {
        return false;
